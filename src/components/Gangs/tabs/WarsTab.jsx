...
export function useBetStake(war, canSee) {
  ...
  const { data } = useDocData(
    canSee && id ? path(`wars/${id}/secret/stake`) : null,
  );
...
function PendingBetCard({ war, us, rank, children }) {
  const pair = {
    ...
    names: Object.fromEntries(
      war.gangIds.map((g) => [g, war.sides?.[g]?.name]),
    ),
    logos: Object.fromEntries(
      war.gangIds.map((g) => [g, war.sides?.[g]?.logo]),
    ),
  };
  ...
  return (
    <Card className={`gx-betcard${offered ? " waiting" : " accepted"}`}>
      <div className="gx-betcard-tag">
        {offered ? "🎲 Bahis teklifi" : "🎲 Bahisli savaş"}
      </div>
      <BetPair {...pair} />
      ...
      <div className="gx-betcard-foot">
        {offered ? (
          <LiveCountdown
            untilMs={answerUntil}
            label="⏳ Cevap için"
            doneLabel="⌛ Süre doldu"
          />
        ) : (
          <LiveCountdown
            untilMs={war.startsAtMs}
            label="⚔️ Başlamasına"
            doneLabel="⚔️ Başladı!"
          />
        )}
...
export function warIcon(w) {
  return (
    { trade: "⚔️", bet: "🎲", sabotage: "💣", defense: "🛡️", intelop: "🎯" }[
      w.type
    ] || "⚔️"
  );
}
...
function warPurpose(w) {
  const p = productOf(w.product);
  if (w.type === "trade")
    return `🎯 Kazanan ${p.emoji} ${p.label} ticaret yolunu ${GANG_RULES.ROUTE_DAYS} gün alır; günlük sipariş limiti = kullandığı gücün %5'i. İstihbarat kazanırsa yol kimseye verilmez, kasasına gücün 1/10'u girer. Kaybedenler katkıları kadar prestij kazanır.`;
  if (w.type === "bet")
    return "🎯 Kazanan tüm bahsi alır (iki çetenin bahsi toplamı).";
  if (w.type === "sabotage")
    return "🎯 En güçlü saldırı savunmayı geçerse tırın yükü saldıranın deposuna gider. Tır her durumda sahibine döner.";
  if (w.type === "intelop")
    return "🎯 Başarılı operasyonda yük imha edilir, İstihbarat anlık satış değerini ödül alır.";
  if (w.type === "defense")
    return "🎯 Savunma gücü en güçlü saldırıdan düşük kalırsa yük kaybedilir. Müttefikler savunmaya güç ekler (saldırıda eklenmez).";
  return "";
}

function WarDetail({ war, onClose, mySideKey }) {
  const { path } = useGang();
  const { docs: rolls } = useQueryData(
    path(`wars/${war.id}/rolls`),
    () => [orderBy("contribution", "desc"), limit(10)],
    war.id,
  );
  ...
  return (
    <Sheet title={warTitle(war)} icon={warIcon(war)} onClose={onClose}>
      ...
      {sides.map((s, i) => (
        <div
          key={s.key}
          className={`gx-rank-row${s.key === mySideKey ? " mine" : ""}`}
        >
          ...
          <div className="gx-rank-main">
            ...
            <Bar
              ...
              max={max}
              color={
                s.key === mySideKey ? "var(--neon-yellow)" : "var(--neon-cyan)"
              }
              height={6}
            />
          </div>
...
function WarRow({
  ...
  const reason =
    disabledReason ||
    (notStarted ? `${fmtClock(war.startsAtMs)}` : slotUsed ? "Katıldın" : null);
  return (
    <Card
      className={`gx-warrow gx-war-${war.type}${tone ? ` ${tone}` : ""}`}
      onClick={onOpen}
    >
      <div className="gx-warrow-main">
        <div className="gx-war-top">
          ...
          <span className={`gx-timer${notStarted ? " wait" : ""}`}>
            ⏱ {fmtCountdown(left)}
          </span>
        </div>
        {children}
      ...
      {onJoin && (
        <div className="gx-warrow-join" onClick={(e) => e.stopPropagation()}>
          <button
            className="gx-join-btn"
            onClick={onJoin}
            disabled={Boolean(reason)}
            title={reason || joinLabel}
          >
            {reason ? (
              <>
                <span>🔒</span>
  ...
  return (
    <div className="gx-three">
      {sides.map((s) => (
        <div
          key={s.key}
          className={`gx-three-row${s.key === mine ? " mine" : ""}`}
        >
...
export function joinableWars(L, now) {
  ...
  return all.filter(
    (w) => now >= (w.startsAtMs || 0) && now < (w.endsAtMs || 0),
  );
}
...
function AttackersSheet({ def, attackers, lead, onClose }) {
  ...
  const payUntil =
    istMidnight(istDateKey(now)) + GANG_RULES_HARAC_HOUR * 3600_000;
  return (
    <Sheet
      title={`TIR #${def.truckCode} saldırganları`}
      icon="⚠️"
      onClose={onClose}
    >
      ...
      {ask && (
        <Confirm
          ...
          title={
            ask.type === "intelop"
              ? `İstihbarata ${fmt(ask.bribe)} rüşvet ödensin mi?`
              : `${ask.sides?.attacker?.name} çetesine ${fmt(ask.harac)} haraç ödensin mi?`
          }
          lines={[
            `💸 ${fmt(ask.type === "intelop" ? ask.bribe : ask.harac)} çete kasasından`,
            ask.type === "intelop"
              ? "🕵️ Para İstihbarat kasasına gider"
              : `🏴 Para ${ask.sides?.attacker?.name || "saldıran"} kasasına gider`,
            ask.type === "intelop"
              ? "⚔️ Operasyon durur, tır güvende"
              : "⚔️ Bu saldırı durur, diğerleri sürer",
          ]}
          confirmLabel="Öde"
          busy={Boolean(busy)}
          ...
          onConfirm={async () => {
            await run(
              ...
              { warId: ask.id },
              {
                success:
                  ask.type === "intelop"
                    ? "💼 Rüşvet ödendi, operasyon durdu"
                    : "🤑 Haraç ödendi, saldırı durdu",
              },
            );
            setAsk(null);
          }}
...
function OfferCard({ kind, item, rank }) {
  ...
  return (
    <Card
      className={kind === "bet" ? "gx-offer gx-betcard waiting" : "gx-offer"}
    >
      {kind === "bet" ? (
        <>
          ...
          <BetPair
            ...
            names={Object.fromEntries(
              item.gangIds.map((g) => [g, item.sides?.[g]?.name]),
            )}
            logos={Object.fromEntries(
              item.gangIds.map((g) => [g, item.sides?.[g]?.logo]),
            )}
          />
        </>
      ) : (
      ...
      {kind === "bet" && (
        <div className="gx-betcard-row">
          ...
          <LiveCountdown
            untilMs={istMidnight(item.offeredDateKey) + 24 * 3600_000}
            label="⏳ Cevap için"
            doneLabel="⌛ Süre doldu"
          />
        </div>
        ...
        <div className="gx-row-2">
          <Btn
            ...
            onClick={() =>
              run(
                "suggest",
                { kind, refId, choice: "reject" },
                { key: `sr_${refId}`, success: "💬 Önerin sohbete gönderildi" },
              )
            }
          >
            💬 Reddedelim
          ...
          <Btn
            ...
            onClick={() =>
              run(
                "suggest",
                { kind, refId, choice: "accept" },
                { key: `sa_${refId}`, success: "💬 Önerin sohbete gönderildi" },
              )
            }
          >
            💬 Kabul edelim
          ...
          onConfirm={async () => {
            if (kind === "bet")
              await run(
                ...
                {
                  success:
                    ask === "accept" ? "⚔️ Bahis kabul edildi" : "Reddedildi",
                },
              );
            else
              await run(
                ...
                {
                  success:
                    ask === "accept" ? "🤝 İttifak kabul edildi" : "Reddedildi",
                },
              );
            setAsk(null);
          }}
...
function ProposeSheet({ kind, gangId, out = [], blockedIds = [], onClose }) {
  ...
  const others = gangs.filter(
    (g) =>
      g.id !== gangId && g.status === "active" && !blockedIds.includes(g.id),
  );
  const pending = out.find((w) => w.status === "offered");
  const acceptedToday = out.find(
    (w) => w.status === "accepted" && w.offeredDateKey === today,
  );
  const { stake: shownStake } = useBetStake(
    kind === "bet" ? pending || acceptedToday : null,
    true,
  );
  const pick = async (g) => {
        ...
        {!GANG_RULES.BET_OFFER_WEEKDAYS.includes(weekday) && !shown ? (
          ...
          <div className={`gx-bet-target${pending ? " waiting" : " accepted"}`}>
            ...
            <div className="gx-bet-target-main">
              ...
              <span className="gx-bet-stake">
                🎲 {shownStake != null ? fmt(shownStake) : "…"}
              </span>
              <span className="gx-bet-status">
                {pending ? (
                  <LiveCountdown
                    untilMs={
                      istMidnight(pending.offeredDateKey) + 24 * 3600_000
                    }
                    label="⏳ Cevap için"
                    doneLabel="⌛ Süre doldu"
                  />
          ...
          <div className="gx-bet-list">
            {others.length === 0 && (
              <Empty icon="🏴" text="Teklif edilebilecek çete yok." />
            )}
            {others.map((g) => {
    ...
    <Sheet title="İttifak teklif et" icon="🤝" onClose={onClose}>
      <div className="gx-pick-list">
        {others.map((g) => (
          <button
            key={g.id}
            className={`gx-pick${target?.id === g.id ? " active" : ""}`}
            onClick={() => pick(g)}
          >
        ...
        <Card key={d.id} className="gx-claim">
          <div>
            ...
            <div className="dim gx-mini">
              {d.createdByName} · {d.claimedCount || 0}/{d.slots} alındı · ⏱{" "}
              {fmtCountdown(d.expiresAtMs - now)}
            </div>
          </div>
          <Btn
            busy={busy === `cl_${d.id}`}
...
export default function WarsTab({ org, d }) {
  ...
  const lead = isIntel
    ? INTEL_LEADERS.includes(d.rank)
    : LEADERS.includes(d.rank);
  const wars = d.wars;

  const doRoll = useCallback(
    () => call("rollDice", rolling.payload),
    [call, rolling],
  );
  const join = (war, payload, side) =>
    setRolling({ war, payload: { warId: war.id, ...payload }, side });
  ...
  const L = useMemo(() => {
    ...
    const liveBets = wars.all.filter(
      (w) =>
        w.type === "bet" &&
        (w.status === "active" ||
          (w.status === "accepted" && nowMin >= w.startsAtMs)) &&
        nowMin < w.endsAtMs,
    );
    ...
    const attacksOnDef = (def) =>
      wars.all
        .filter(
          (w) =>
            (w.type === "sabotage" || w.type === "intelop") &&
            w.defenseWarId === def.id &&
            w.status === "active",
        )
        .sort(
          (a, b) => (b.display?.attacker || 0) - (a.display?.attacker || 0),
        );
    if (isIntel)
      return {
        trade,
    ...
    return {
      ...
      myAttacks: active.filter(
        (w) => w.type === "sabotage" && w.attackerGangId === gangId,
      ),
      myDefs: active.filter(
        (w) =>
          w.type === "defense" && w.defenderGangId === gangId && w.announced,
      ),
      allyDefs: active.filter(
        (w) =>
          w.type === "defense" && allyIds.has(w.defenderGangId) && w.announced,
      ),
      betOffersIn: wars.all.filter(
        (w) =>
          w.type === "bet" &&
          w.status === "offered" &&
          w.targetGangId === gangId,
      ),
      betOffersOut: wars.all.filter(
        (w) =>
          w.type === "bet" &&
          (w.status === "offered" ||
            (w.status === "accepted" && nowMin < w.startsAtMs)) &&
          w.proposerGangId === gangId,
      ),
      betsAccepted: wars.all.filter(
        (w) =>
          w.type === "bet" &&
          w.status === "accepted" &&
          nowMin < w.startsAtMs &&
      ...
      allianceIn: d.alliances.filter(
        (a) => a.status === "requested" && a.requestedBy !== gangId,
      ),
    };
  ...
  const allyBlocked = isIntel
    ? []
    : d.alliances
        .filter((a) =>
          ["requested", "accepted", "active", "ending"].includes(a.status),
        )
        .flatMap((a) => a.gangIds);
  const betBlocked = isIntel
    ...
    : [
        ...d.alliances
          .filter((a) => ["accepted", "active", "ending"].includes(a.status))
          .flatMap((a) => a.gangIds),
        ...wars.all
          .filter(
            (w) =>
              w.type === "bet" &&
              ["offered", "accepted", "active"].includes(w.status),
          )
          .flatMap((w) => w.gangIds || []),
      ];
  ...
  return (
    <div className="gx-stack">
      {!isIntel &&
        d.membership.intelDecisionGangId === gangId &&
        d.membership.intelRosterId &&
        d.rank === "baba" && <IntelDecisionPanel />}
      ...
      {!isIntel && lead && (
        <div className="gx-row-2">
          <Btn small kind="ghost" onClick={() => setPropose("bet")}>
            🎲{" "}
            {pendingOut
              ? `⏳ ${pendingOut.sides?.[pendingOut.targetGangId]?.name || ""}`
              : "Bahis teklif et"}
          </Btn>
          ...
          <WarRow
            ...
            onJoin={() =>
              join(
                w,
                isIntel ? { side: "intel" } : {},
                isIntel ? "İstihbarat" : d.gang?.name,
              )
            }
          >
      ...
      {(L.bets || []).map((w) => {
        const other = Object.keys(w.sides || {}).find(
          (k) => k !== gangId && k !== "intel",
        );
        ...
        return (
          ...
          >
            {w.sides?.intel ? (
              ...
              <Tug
                us={w.display?.[gangId] || 0}
                them={w.display?.[other] || 0}
                usLabel={d.gang?.name}
                themLabel={w.sides?.[other]?.name}
              />
            )}
            <div className="gx-mini">
              <BetAmount
                war={w}
                canSee={!isIntel && BET_RANKED.includes(d.rank)}
                pot
                note={!isIntel}
              />
            </div>
        ...
        return (
          ...
          >
            {top ? (
              <div className="gx-top-attacker">
                ...
                <b>
                  {top.type === "intelop"
                    ? "🕵️ İstihbarat"
                    : top.sides?.attacker?.name}
                </b>{" "}
                ⚔️ {fmt(topPower)}
                {atk.length > 1 && (
                  <span className="dim"> · +{atk.length - 1} saldırgan</span>
                )}
              </div>
            ) : (
            ...
            <div className="gx-vs">
              ...
              <span className={defTotal < topPower ? "bad" : "good"}>
                ⚔️ {fmt(topPower)}
              </span>
            </div>
      ...
      {(L.myAttacks || []).map((w) => {
        ...
        return (
          <WarRow
            ...
            war={w}
            slotUsed={slot.used}
            onOpen={() => setDetail({ war: w, side: "attacker" })}
            onJoin={() => join(w, {}, d.gang?.name)}
            joinLabel="SALDIR"
          >
            ...
            {w.harac > 0 && (
              <div className="gx-war-purpose">🤑 {fmt(w.harac)}</div>
            )}
          </WarRow>
        );
      })}
      ...
      {!isIntel &&
        L.betOffersIn.map((w) => (
          <OfferCard key={w.id} kind="bet" item={w} rank={d.rank} />
        ))}
      {!isIntel &&
        L.allianceIn.map((a) => (
          <OfferCard key={a.id} kind="alliance" item={a} rank={d.rank} />
        ))}
      {!isIntel &&
              ...
              <Btn
                ...
                onClick={() =>
                  run(
                    "withdrawBet",
                    { warId: w.id },
                    { key: `wd_${w.id}`, success: "Teklif geri çekildi" },
                  )
                }
              >
                Geri çek
      ...
      {d.pending.map((p) => (
        <Card key={p.id} className="gx-pending">
          <span>
            🤫{" "}
            {isIntel
              ? `${p.targetCode} için çıkarma`
              : p.type === "kick"
                ? `${p.targetName} için çıkarma`
                : p.type === "devirme"
                  ? "Devirme"
                  : "Ayaklanma"}{" "}
            · ⏱ 00:00
          ...
          <Btn
            ...
            onClick={() =>
              run(
                isIntel ? "cancelIntelVoteRequest" : "cancelVoteRequest",
                { pendingId: p.id },
                { key: `c_${p.id}`, success: "İptal edildi" },
              )
            }
          >
            İptal
      ...
      {d.votes.map((v) =>
        isIntel ? (
          <VoteCard
            key={v.id}
            vote={v}
            ballotPath={path(`intel/main/votes/${v.id}/ballots/${d.rid}`)}
            voterKey={d.rid}
            action="castIntelVote"
          />
        ) : (
          <VoteCard
            key={v.id}
            vote={v}
            ballotPath={path(
              `gangs/${gangId}/votes/${v.id}/ballots/${actorId}`,
            )}
            voterKey={actorId}
            action="castVote"
          />
      ...
      <ClaimList
        ...
        rank={d.rank}
        isIntel={isIntel}
        myKey={isIntel ? d.rid : actorId}
        joinedAtMs={isIntel ? d.me?.joinedAtMs : d.me?.joinedAtMs}
      />

      {detail && (
        <WarDetail
          war={wars.all.find((w) => w.id === detail.war.id) || detail.war}
          mySideKey={detail.side}
      ...
      {rolling && (
        <DiceRoller
          title={`${warIcon(rolling.war)} ${warTitle(rolling.war)}`}
          subtitle={rolling.side}
          sideLabel={rolling.side}
          onRoll={doRoll}
